...
def val(model, val_loader, loss_func, n_labels):
    ...
    val_log = OrderedDict({'Val_Loss': val_loss.avg, 'Val_dice_rib': val_dice.avg[1]})
...
def train(model, train_loader, optimizer, loss_func, n_labels, alpha):
    ...
    train_log = OrderedDict({'Train_Loss': train_loss.avg, 'Train_dice_rib': train_dice.avg[1]})
    if n_labels==3: val_log.update({'Train_dice_tumor': train_dice.avg[2]})
    return train_log

if __name__ == '__main__':
    args = config.args
    save_path = os.path.join('./Homework_experiments', args.save)
    ...
    model.apply(weights_init.weights_init_normal)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        trigger += 1
        if val_log['Val_dice_rib'] > best[1]:
            print('Saving best model')
            torch.save(state, os.path.join(save_path, 'best_model.pth'))
            best[0] = epoch
            best[1] = val_log['Val_dice_rib']
            trigger = 0
        print('Best performance at Epoch: {} | {}'.format(best[0],best[1]))

        # 深监督系数衰减
        if epoch % 30 == 0: alpha *= 0.9
